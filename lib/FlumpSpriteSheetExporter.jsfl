	...
	SpriteSheetWriter.prototype = {
		...
		write: function () {
			...
			while (i++ < 10 && (this.tooLargeSprites.length || this.tooLargeFlipbooks.length)) {
				...
				var spriteNames = this.tooLargeSprites.map(function (sprite) {
					return sprite.name.slice(0, -1);
				});
				var flipbookNames = this.tooLargeFlipbooks.map(function (flipbook) {
					return flipbook.name.slice(0, -1);
				});
